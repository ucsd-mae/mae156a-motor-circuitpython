...
M1A = pwmio.PWMOut(motor1_pwmA, frequency=10000)
M1B = pwmio.PWMOut(motor1_pwmB, frequency=10000)
...
while True:
    ...
    if new_value != last_value:
        print("systime {:5.5f}, throttle {:s}, position {:8d}, delta encoder {:3d}, delta t(s) {:1.7f}, raw rpm {:4.0f}, filtered rpm {:4.0f}"\
    ...
    time.sleep(sleep_seconds) # slow down update loop, encoder counts unaffected, pwm output to motor unaffected
